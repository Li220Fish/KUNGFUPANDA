...
const SignUp = ({ isOpen, onClose }) => {
  const [currentStep, setCurrentStep] = useState(1); // 控制步驟
  const [isModalOpen, setIsModalOpen] = useState(isOpen); // 管理小視窗是否開啟

  // 當父組件的 isOpen 改變時，更新小視窗的狀態
  useEffect(() => {
  ...
  const handleButtonClick = () => {
    setCurrentStep(2); // 切換到 SignUp2.js
  };

  // 關閉小視窗的處理邏輯
  const handleClose = () => {
    setIsModalOpen(false); // 關閉小視窗
    setCurrentStep(1); // 重置步驟
    onClose(); // 通知父組件關閉
  };
        ...
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <h3 className="policy-title">
              註冊即表示你同意並接受我們的 條款細則 及 隱私權政策 。
            </h3>
          </div>
        </div>
      )}
{currentStep === 2 && <SignUp2 onBack={() => setCurrentStep(1)} onClose={handleClose} />}
</>
  ) : null;
};
